...
export const Card = ({  item: {image, rating, name, address, price }, onPress }: Props) => {
  return (
    <TouchableOpacity
      ...
      onPress={onPress}
    >
      <View className="flex flex-row items-center bg-white/90 absolute px-2 top-5 right-5 p-1 rounded-full z-50">
        <Image className="size-2.5" source={icons.star} />
        <Text className="text-xs font-rubik-bold text-primary-300 ml-safe-or-0.5">
          {rating}
        </Text>
      </View>
